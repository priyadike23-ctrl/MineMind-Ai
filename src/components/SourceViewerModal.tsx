  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-2 sm:p-4 backdrop-blur-xs">
      <div className="bg-white rounded-xl max-w-3xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-[#E4E0D6] overflow-hidden">
        ...
        <div className="p-4 sm:p-5 bg-[#141C2B] text-white border-b border-[#1E293B]">
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-1 min-w-0 flex-1">
              <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-[#D97706]">
              ...
              <h3 className="font-sans font-bold text-base sm:text-lg text-white mt-1 break-words">
                {activeCitationForModal.documentTitle}
              </h3>
            </div>
            <button
              onClick={() => setActiveCitationForModal(null)}
              className="text-[#94A3B8] hover:text-white p-1.5 rounded-xl hover:bg-[#1E293B] transition-colors cursor-pointer flex-shrink-0"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* Modal Body */}
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 bg-[#F0F4F8] flex-1 text-xs">
          {/* Metadata Cards */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 font-mono">
            <div className="bg-white p-3 rounded-xl border border-[#CBD5E1]">
              ...
              <span className="font-bold text-[#0B2238] mt-0.5 block">{activeCitationForModal.subsidiary}</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-[#CBD5E1]">
              <span className="text-[10px] text-[#64748B] block">Version:</span>
              <span className="font-bold text-[#0B2238] mt-0.5 block">v{activeCitationForModal.versionNumber}.0 (Approved)</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-[#CBD5E1]">
              <span className="text-[10px] text-[#64748B] block">File Location:</span>
              <span className="font-bold text-[#00529B] mt-0.5 block">{activeCitationForModal.pageOrSheetRef}</span>
            </div>
            <div className="bg-white p-3 rounded-xl border border-[#CBD5E1]">
              <span className="text-[10px] text-[#64748B] block">Relevance Match:</span>
              <span className="font-bold text-[#047857] mt-0.5 block">{((activeCitationForModal.relevanceScore || 0.98) * 100).toFixed(0)}% Match</span>
            </div>
          </div>

          {/* Highlighted Excerpt Chunk */}
          <div className="bg-white p-4 sm:p-5 rounded-2xl border-2 border-[#00529B] shadow-xs space-y-2.5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs font-mono">
              <span className="font-bold text-[#00529B] uppercase tracking-wider flex items-center gap-1.5">
                <BookOpen className="w-4 h-4" />
                <span>Cited Text Chunk from Official Record</span>
              </span>
              <button
                onClick={handleCopy}
                className="px-2.5 py-1 bg-[#F8FAFC] hover:bg-[#E2E8F0] border border-[#CBD5E1] rounded-lg text-[11px] font-semibold text-[#0B2238] flex items-center gap-1 self-start sm:self-auto cursor-pointer"
              >
                {copied ? <Check className="w-3 h-3 text-[#047857]" /> : <Copy className="w-3 h-3 text-[#64748B]" />}
                <span>{copied ? 'Copied' : 'Copy Excerpt'}</span>
              </button>
            </div>

            <div className="p-3.5 bg-[#F8FAFC] rounded-xl border border-[#CBD5E1] font-sans text-xs sm:text-sm leading-relaxed text-[#0B2238]">
          ...
          {targetVer?.keyMetrics && targetVer.keyMetrics.length > 0 && (
            <div className="bg-white p-4 rounded-2xl border border-[#CBD5E1] space-y-3">
              <h4 className="font-sans font-bold text-xs text-[#0B2238]">
