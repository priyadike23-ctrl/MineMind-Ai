...
export const LoginScreen: React.FC = () => {
  ...
  const handleGoogleSuccessLogin = (profile: { name: string; email: string; picture?: string }) => {
    ...
    const googleUser: AppUser = {
      ...
      name: profile.name || knownUser?.name || userEmail.split('@')[0],
      email: userEmail,
  ...
  // Handle Google OAuth Sign-in: Direct Google Cloud Console GIS first, then Supabase, or opens Assistant Modal
  const handleGoogleSignIn = async () => {
    ...
    // 2. If a Google Client ID exists, directly trigger official Google popup
    if (clientId) {
      try {
        setIsSubmitting(true);
        ...
        console.warn('[Google Auth] Direct Google GIS flow notice:', directErr);
        const errText = directErr?.message || 'Google authorization was cancelled or encountered a origin configuration mismatch.';
        setGoogleConsoleError(errText);
    ...
    // 3. If Supabase is configured with Google provider, attempt Supabase OAuth
    const client = getSupabase() || supabase;
    if (client) {
      try {
        setIsSubmitting(true);
        ...
        if (error) {
          console.warn('[Supabase Google] Error:', error.message);
          setGoogleConsoleError(error.message || 'Supabase Google sign-in failed. Please verify your Google Console OAuth settings.');
          setIsGoogleConsoleModalOpen(true);
          setIsSubmitting(false);
          return;
        }
        ...
        console.warn('[Supabase Google] Exception:', err);
        setGoogleConsoleError(err?.message || 'An unexpected error occurred during Google sign-in.');
        setIsGoogleConsoleModalOpen(true);
        setIsSubmitting(false);
        return;
      }
    ...
    // 4. If neither Google Client ID nor Supabase is configured yet, launch the interactive Google Console Assistant!
    setIsGoogleConsoleModalOpen(true);
  };
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center justify-center">
                  <button
                    ...
                    className="text-[11px] text-[#00529B] hover:text-[#0B2238] hover:underline font-semibold flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    <Settings className="w-3 h-3 text-[#D97706]" />
                    <span>Google Cloud Console Setup &amp; Direct Access</span>
                  </button>
                </div>
              </div>
